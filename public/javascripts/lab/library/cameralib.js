var Camera = function(finderFrameSelector, pictureFrameSelector, options){
	...
	var defaults = {
		...
		square : false,
		successCallback : function(stream) {console.dir(stream);},
		errorCallback : function(error){console.dir(error);}
	}

	if(options){

		if(options.square){defaults.square = options.square;}
		if(options.successCallback){defaults.successCallback = options.successCallback;}
		if(options.errorCallback){defaults.errorCallback = options.errorCallback;}
	}
	...
	this.take = function(takeCallback){
		...
		capture.takePhoto(
			...
			context.drawImage(bitmap,
				cropVideoX, cropVideoY,
				cropVideoWidth, cropVideoHeight,
				pictureX, pictureY,
				pictureWidth, pictureHeight);
