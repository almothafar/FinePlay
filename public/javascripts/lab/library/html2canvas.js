...
$(document).ready(function() {
	...
	var chart = LightweightCharts.createChart(chartFrame, {
	...
	window.onresize = function() {
		...
		chart.resize($(chartFrame).height(), $(chartFrame).width());
	};
	...
	$('#captureButton').on('click', function(){
		...
		html2canvas($('#chartFrame>.tv-lightweight-charts>table')[0]).then(function(canvas) {

			var dataURI = canvas.toDataURL();
			$('#captures').prepend(
				'<div class="col-12 col-md-6">' +
					'<img class="w-100" src="' + dataURI + '"></img>' +
				'</div>'
			);
		});
	});
