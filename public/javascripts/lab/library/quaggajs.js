...
$(function() {
	...
	var App = {
			...
			return Quagga.CameraAccess
					...
					.then(
							function(devices) {
								...
								var $deviceSelection = document
										.getElementById("deviceSelection");
								while ($deviceSelection.firstChild) {
									$deviceSelection
											.removeChild($deviceSelection.firstChild);
								}
								devices
										.forEach(function(device) {
											...
											$option
													.appendChild(document
															.createTextNode(pruneText(device.label
																	|| device.deviceId
																	|| device.id)));
		...
		attachListeners : function() {
			...
			$(".controls .reader-config-group")
					.on(
							...
							function(e) {
								e.preventDefault();
								var $target = $(e.target), value = $target
										.attr("type") === "checkbox" ? $target
										.prop("checked") : $target.val(), name = $target
										.attr("name"), state = self
		...
		_accessByPath : function(obj, path, val) {
			var parts = path.split('.'), depth = parts.length, setter = (typeof val !== "undefined") ? true
					: false;
	...
	Quagga.onDetected(function(result) {
		...
		if (App.lastResult !== code) {
			...
			'<div class="alert bg-orange alert-dismissible fade show p-0 d-flex justify-content-start" role="alert">' +
				'<div class="p-2"><img src="' + canvas.toDataURL() + '" class="w-96p h-96p rounded"></div>' +
				'<div class="p-2 align-self-stretch w-100"><strong class="text-blue-grey">Code</strong><br>' + code + '</div>' +
