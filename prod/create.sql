...
alter table if exists COMPANIES_COMPANY_NAMES add constraint UK_iesn9bki7ydmkpr1n55aexfne unique (names_company_Id, names_locale)
...
alter table if exists ORGANIZATION_UNITS add constraint UKejy8rsqnmvdocs3wy7mtbo8ld unique (id, organization_id)
alter table if exists ORGANIZATION_UNITS_ORGANIZATION_UNIT_NAMES add constraint UK_5hiq31t2io2yup2ul08dabu6q unique (names_locale, names_organizationUnit_Id)
...
alter table if exists ORGANIZATIONS add constraint UK8wgif8llthydveottfhshbwdu unique (id, company_id)
alter table if exists ORGANIZATIONS add constraint UK_1akjfyqylxqvygx3326h6j2c unique (company_id)
create index IDX207cn8vnfsr9hdx9f0a9y2hlj on USERS (userId)
alter table if exists USERS add constraint UKqiqomcfkvxicts5dvpm0swvvl unique (id, userId)
alter table if exists USERS add constraint UK_207cn8vnfsr9hdx9f0a9y2hlj unique (userId)
alter table if exists COMPANIES_COMPANY_NAMES add constraint FK7dplmqp7bb1m74fik1aoxy0yq foreign key (names_company_Id, names_locale) references COMPANY_NAMES
alter table if exists COMPANIES_COMPANY_NAMES add constraint FKn3qhtm3r4eu9cd78si05xrv5h foreign key (Company_id) references COMPANIES
alter table if exists ORGANIZATION_UNITS add constraint FKc36lxm5lc90vm1kque8vr7cyd foreign key (organization_id) references ORGANIZATIONS
alter table if exists ORGANIZATION_UNITS add constraint FKaxl5lltie4tsoy0wug3btb5hl foreign key (PARENT_ID) references ORGANIZATION_UNITS
alter table if exists ORGANIZATION_UNITS_ORGANIZATION_UNIT_NAMES add constraint FKpqciiy5y9smeeod0xvanq6wfg foreign key (names_locale, names_organizationUnit_Id) references ORGANIZATION_UNIT_NAMES
alter table if exists ORGANIZATION_UNITS_ORGANIZATION_UNIT_NAMES add constraint FKciddj53m25d6p61rhine0fvbo foreign key (OrganizationUnit_id) references ORGANIZATION_UNITS
alter table if exists ORGANIZATIONS add constraint FKpl7c9r2u24rjr6pqmrcd9mxdq foreign key (company_id) references COMPANIES
alter table if exists USER_ROLES add constraint FKk3qtke64s9k5pv5hoq6yyq7py foreign key (USER_ID) references USERS
alter table if exists USERS add constraint FKdeidj66mcx6k8vsoflveak3gj foreign key (COMPANY_ID) references COMPANIES
